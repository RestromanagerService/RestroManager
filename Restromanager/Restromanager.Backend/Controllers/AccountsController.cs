...
namespace Restromanager.Backend.Controllers
{
    ...
    public class AccountsController(IUserUnitOfWork unitOfWork, IConfiguration configuration, IFileStorage fileStorage,IMailHelper mailHelper) : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateUserAsync([FromBody] UserDTO model)
        {
            ...
            var result = await _unitOfWork.AddUserAsync(user, model.Password);
            if (result.Succeeded)
            {
                await _unitOfWork.AddUserToRoleAsync(user, user.UserType.ToString());
        ...
        {
            token = token.Replace(" ", "+");
            var user = await _unitOfWork.GetUserAsync(new Guid(userId));
            if (user == null)
            {
                return NotFound();
            }
        ...
        {
            var user = await _unitOfWork.GetUserAsync(model.Email);
            if (user == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> ChangePasswordAsync(ChangePasswordDTO model)
        {
            ...
            var user = await _unitOfWork.GetUserAsync(User.Identity!.Name!);
            if (user == null)
            {
                return NotFound();
            }

            var result = await _unitOfWork.ChangePasswordAsync(user, model.CurrentPassword, model.NewPassword);
            if (!result.Succeeded)
            {
        ...
        {
            var result = await _unitOfWork.LoginAsync(model);
            if (result.Succeeded)
            {
                var user=await _unitOfWork.GetUserAsync(model.Email);
        ...
        public async Task<IActionResult> RecoverPasswordAsync([FromBody] EmailDTO model)
        {
            ...
            var myToken = await _unitOfWork.GeneratePasswordResetTokenAsync(user);
        ...
        public async Task<IActionResult> ResetPasswordAsync([FromBody] ResetPasswordDTO model)
        {
            ...
            var result = await _unitOfWork.ResetPasswordAsync(user, token, model.Password);
            if (result.Succeeded)
            {
                return NoContent();
            }
        ...
        public async Task<IActionResult> PutAsync(User user)
        {
            try
            {
                var currentUser = await _unitOfWork.GetUserAsync(User.Identity!.Name!);
                if (currentUser == null)
                {
                    return NotFound();
                }
                ...
                var result = await _unitOfWork.UpdateUserAsync(currentUser);
                if (result.Succeeded)
                {
        ...
        {
            return Ok(await _unitOfWork.GetUserAsync(User.Identity!.Name!));
        }
        ...
        {
            var myToken = await _unitOfWork.GenerateEmailConfirmationTokenAsync(user);
            var tokenLink = $"http://{_configuration["Url Frontend"]}/user/confirmEmail?userid={user.Id}&token={myToken}";
            return _mailHelper.SendMail(user.FullName, user.Email!,
                $"RestroManager - Confirmación de cuenta",
                $"<h1>RestroManager - Confirmación de cuenta</h1>" +
